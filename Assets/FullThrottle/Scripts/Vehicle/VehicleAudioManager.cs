...
public class VehicleAudioManager : MonoBehaviour
{
    ...
    [Range(0,1)]
    public float volumeMultiplier = 1f;                                          // Used for altering the pitch of audio clips
    ...
    private VehicleController vehicleController; // Reference to car we are controlling
    ...
    private void Update()
    {
        ...
        if (m_StartedSound)
        {
            ...
            float pitch = ULerp(lowPitchMin, lowPitchMax, vehicleController.Revs);
            ...
            if (engineSoundStyle == EngineAudioOptions.Simple)
            {
                ...
                m_HighAccel.volume = 1 * volumeMultiplier;
            }
            else
            {
                ...
                float accFade = Mathf.Abs(vehicleController.AccelInput);
                float decFade = 1 - accFade;

                // get the high fade value based on the cars revs
                float highFade = Mathf.InverseLerp(0.2f, 0.8f, vehicleController.Revs);
                ...
                m_LowAccel.volume = lowFade * accFade* volumeMultiplier;
                m_LowDecel.volume = lowFade * decFade* volumeMultiplier;
                m_HighAccel.volume = highFade * accFade* volumeMultiplier;
                m_HighDecel.volume = highFade * decFade* volumeMultiplier;
